/**
 * Solitaire entry point. Wires the controller to the renderer and the chrome.
 */
...
import { GAME_ID, type GameState, SolitaireGame } from './game';
...
app.classList.add('app--solitaire');
...
/**
 * Seven columns across the screen is the whole scale of this game.
 *
 * The card width falls out of the width available, the height out of the width,
 * and the tableau gets whatever is left under the reserve row. The gaps are
 * *measured* rather than assumed, because a copy of a CSS value kept in JS is
 * how Gridlock once hung its exit five pixels off the screen.
 *
...
function fitBoard(): boolean {
  ...
  const tableauHeight = Math.max(height, availableHeight - height - rowGap);
...
function handleEffect(state: GameState): void {
  ...
  else if (effect.kind === 'turn') sfx.select();
  else if (effect.kind === 'reject') sfx.reject();
  else if (effect.kind === 'play') {
    // Pitch rises with how many cards that move sent home, so a move that
    // unlocks a run is audible without anything having to announce it.
    if (effect.home > 0) sfx.complete();
    else sfx.pour(2);
  }
}
...
function showWin(state: GameState): void {
  openSheet(
    (sheet) => {
      sheet.content.append(el('h2', { class: 'win-title' }, 'All out'));
...
function showLoss(): void {
  openSheet(
    (sheet) => {
      sheet.content.append(el('h2', { class: 'lose-title' }, 'No way out'));
...
hintButton.addEventListener('click', () => {
  // Null is the search admitting it ran out of budget rather than out of
  // positions. There is nothing honest to show for that, so it buzzes.
...
settingsButton.addEventListener('click', () => {
  openSettings({
    ...
    shapesDescription: 'Diamonds blue, clubs green.',
